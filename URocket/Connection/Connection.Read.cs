...
public sealed unsafe partial class Connection : IValueTaskSource<ReadResult>
{
    ...
    public UnmanagedMemoryManager[] GetAllRingsAsUnmanagedMemory(ReadResult readResult) 
    {
        var count = RingCount;
    ...
    public RingItem[] GetAllRings(ReadResult readResult) 
    {
        var count = RingCount;
    ...
    public UnmanagedMemoryManager[] PeekAllRingsAsUnmanagedMemory(ReadResult readResult) 
    {
        var count = RingCount;

        if (count == 1) 
        {
            TryPeekRing(readResult.TailSnapshot, out var item);
            return [item.AsUnmanagedMemoryManager()];
        }
    ...
    public RingItem[] PeekAllRings(ReadResult readResult) 
    {
        var count = RingCount;
